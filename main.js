...
function main(fileName) {
  const fileToParse = fileName || SHORTCUTS_FILE_NAME;

  // read file
  const filePath = path.join('./', fileToParse);
  ...
  console.log(`>>> XML Data has been parsed successfully`);
  console.log(`>>> Translating shortcuts`);
  ...
  console.log(`>>> Shortcuts translated successfully`);
  ...
  const updatedFileName = fileToParse.replace(/\.xml$/, '-TRANSLATED.xml');
  const updatedFilePath = path.join('./', updatedFileName);

  console.log(`>>> Saving results in ${updatedFileName} under the next path: ${updatedFilePath}`);
  ...
  fs.writeFile(updatedFilePath, resultXml, function (err) {
    if (err) {
      ...
      console.log(`>>> TRANSLATED SHORTCUTS WRITTEN TO ${updatedFilePath}`);
    }
  });
}
